...
import { parseCurses, parseCursesReply, parseCursesMessage } from '@/json/parseCurses';

function isBanned(content: string): boolean {
    const bannedWords: string[] = parseCurses();
    const contentLower: string = content.toLowerCase();
    const hasBannedWord: boolean = bannedWords.some(word => contentLower.includes(word.toLowerCase()));
    return hasBannedWord;
}

async function handleCursesInThreads(thread: any): Promise<void> {
