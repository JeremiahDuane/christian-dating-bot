...
const parseRuleAsString = (tenet: { rule: string; description: string }, index: number): string =>
    `${index + 1}. **${tenet.rule}**: ${tenet.description}`;

const parseRulesAsList = (): string[] =>
    json.tenets.map((tenet: { rule: string; description: string }, index: number) =>
        `${index + 1}. **${tenet.rule}**: ${tenet.description}`);
...
const parseRuleByNumber = (index: number): string => parseRulesAsList()[parseRuleIndex(index)];

const parseBanReplyFromRuleNumberAndUser = (tenet: number, user: { id: string }): string =>
    `<@${user.id}> has been banned for breaking \n ${discordQuote(parseRuleByNumber(tenet))}`;

const parseBanMessageFromRuleNumber = (tenet: number, reason?: string): string =>
